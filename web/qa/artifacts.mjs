...
export default {name:'local-qa-artifacts',configureServer(server){
 server.middlewares.use('/__qa-artifact',async(req,res)=>{
  ...
  try{for await(const chunk of req){size+=chunk.length;if(size>40*1024*1024){res.statusCode=413;res.end();return}chunks.push(chunk)}const destination=name.startsWith('promo-')?resolve('../交付物/个人网站宣传片-20260920'):name.startsWith('story-')?resolve('../交付物/five-designed-worlds-20260921'):name.startsWith('five-')?resolve('../交付物/five-worlds-20260920'):name.startsWith('hyundai-')?resolve('../交付物/hyundai-pilot-20260920'):directory;await mkdir(resolve(destination,'media'),{recursive:true});await writeFile(resolve(destination,'media',name),Buffer.concat(chunks));res.end(name)}catch{res.statusCode=500;res.end('capture failed')}
 })
